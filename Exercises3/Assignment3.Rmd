...
As is seen from the output above, all three tests shows that the model is significantly better than the baseline. The Karnofsky index (`score`) is significant. Moreover, when choosing a significance level of $\alpha = 0.05$, the parameter estimate comparing Hodgkings disease to non Hodgkins disease is significant. 

Since the parameter estimation for the Karnofsky index is negative, the model is predicting that an increase in the Karnofsky score, while all other covariates are kept static, increases the instantaneous risk of death or relapse. On the contrary, having Hodgkins disease is predicted as being protective compared to not having it, when considering the other covariates as static. 

**Skriv opp modellen og tolk skikkelig!**

## 4.d)
Estimate the hazard ratios associated to the graft type and interpret both values.

The hazard ratio measures the comparative instantaneous risk of death.
Since there exists interaction between the graph type and the disease type,
two hazard ratios is computed, one for patients with non Hodgkin lymphoma,
...
instantaneous risk of dying, given that the disease type is non Hodgkin lymphoma
and the Karnofsky index is the same. 
Similarly, patients receiving an autologous graft have 0.326 times lower 
instantaneous risk of dying, given that the disease type is Hodgkins disease
and the Karnofsky index is the same. 
...
Concerning the estimation of the four model parameterse, are there any influential observations?
